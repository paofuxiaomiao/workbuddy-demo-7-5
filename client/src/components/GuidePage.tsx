...
const PDF_CHAPTERS = [
  {
    ...
    content: [
      { type: 'p', text: '把常用提示词单独集中，方便复制、改路径、替换文件名。改写方法：先改路径，再改任务目标，最后改输出结构和保存位置。不要省略：执行前先列计划、不确定先问我、不要覆盖原文件。' },
    ...
    sidebar: { type: 'note', title: '高风险任务清单', items: ['批量移动文件（先复制不直接移动）', '自动化定时执行（先手动跑通）', '远程处理本地文件（写清路径和异常处理）', '建议顺序：手动跑通 → 调整指令 → 再跑一次 → 结果稳定 → 再自动化'] },
  },
  {
    num: 'B', title: '名词解释：简明对照表', color: '#F59E0B',
    content: [
      { type: 'p', text: '用课堂语言解释常见术语，避免把入门课讲成技术课。' },
